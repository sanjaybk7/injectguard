...
```bash
git clone https://github.com/sanjaybk7/agentic-guard.git
cd agentic-guard
...
The taxonomy in `src/agentic-guard/taxonomy.yaml` is the brain of the tool — it
...
A rule lives in `src/agentic-guard/rules/`. The minimal template:

```python
# src/agentic-guard/rules/my_rule.py
from collections.abc import Iterable
from typing import ClassVar

from agentic-guard.ir import Finding, Severity
from agentic-guard.rules.base import Rule, RuleContext
...
2. Register it in `src/agentic-guard/rules/__init__.py` by yielding from
   `all_rules()`.
...
A parser lives in `src/agentic-guard/parsers/` and subclasses `FrameworkParser`.
...
If you find a vulnerability in `agentic-guard` itself (rather than using it to
find vulnerabilities elsewhere), please email the maintainer rather than
opening a public issue.

If you find a vulnerability in a third-party project using `agentic-guard`,
